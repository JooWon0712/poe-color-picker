        ...
        <v-app>
            <v-container class="itemFilterOfficialInfo" style="margin: 4px;">
                <v-row style="margin: 4px;">
                    <div v-bind:style="{ backgroundColor: '#000000', border: '4px solid #000000', color: '#C8C8C8', marginRight: '10px' }"
                        v-clipboard:copy="'C8C8C8'" v-clipboard:success="onCopySuccess" v-clipboard:error="onCopyError">
                        <label>일반 아이템</label>
                    </div>
                    <div v-bind:style="{ backgroundColor: '#000000', border: '4px solid #000000', color: '#8888FF', marginRight: '10px' }"
                        v-clipboard:copy="'8888FF'" v-clipboard:success="onCopySuccess" v-clipboard:error="onCopyError">
                        <label>마법 아이템</label>
                    </div>
                    <div v-bind:style="{ backgroundColor: '#000000', border: '4px solid #000000', color: '#FFFF77', marginRight: '10px' }"
                        v-clipboard:copy="'FFFF77'" v-clipboard:success="onCopySuccess" v-clipboard:error="onCopyError">
                        <label>희귀 아이템</label>
                    </div>
                    <div v-bind:style="{ backgroundColor: '#000000', border: '4px solid #000000', color: '#AF6025', marginRight: '10px' }"
                        v-clipboard:copy="'AF6025'" v-clipboard:success="onCopySuccess" v-clipboard:error="onCopyError">
                        <label>고유 아이템</label>
                    </div>
                    <div v-bind:style="{ backgroundColor: '#000000', border: '4px solid #000000', color: '#4AE63A', marginRight: '10px' }"
                        v-clipboard:copy="'4AE63A'" v-clipboard:success="onCopySuccess" v-clipboard:error="onCopyError">
                        <label>퀘스트 아이템</label>
                    </div>
                    <div v-bind:style="{ backgroundColor: '#000000', border: '4px solid #000000', color: '#0EBAFF', marginRight: '10px' }"
                        v-clipboard:copy="'0EBAFF'" v-clipboard:success="onCopySuccess" v-clipboard:error="onCopyError">
                        <label>점술 카드</label>
                    </div>
                    <div v-bind:style="{ backgroundColor: '#000000', border: '4px solid #000000', color: '#1BA29B', marginRight: '10px' }"
                ...
                <v-row>
                    <v-col>
                        ...
                        <v-text-field v-model="poeBackgroundColor" v-mask="mask" hide-details class="ma-0 pa-0" solo>
                            <template v-slot:append>
                                <v-menu v-model="menu1" nudge-bottom="105" nudge-left="-400"
                    ...
                    <v-col>
                        ...
                        <v-text-field v-model="poeBorderColor" v-mask="mask" hide-details class="ma-0 pa-0" solo>
                            <template v-slot:append>
                                <v-menu v-model="menu2" nudge-bottom="105" nudge-left="-150"
                                    :close-on-content-click="false">
                                    <template v-slot:activator="{ on }">
                    ...
                    <v-col>
                        ...
                        <v-text-field v-model="poeTextColor" v-mask="mask" hide-details class="ma-0 pa-0" solo>
                            <template v-slot:append>
                                <v-menu v-model="menu3" nudge-bottom="105" nudge-left="16"
                                    :close-on-content-click="false">
                                    <template v-slot:activator="{ on }">
            ...
            <v-container>
                <v-row>
                    <v-col class="col md-6">
                        <v-textarea class="custom-textarea" v-model="notePadText" label="메모 공간" outlined
                            rows="5"></v-textarea>
                    </v-col>
            ...
            <v-container class="container-row" style="width: 900px;">
                <v-row style="display: flex; text-align: center; justify-content: center; margin: 40px;">
                    <v-col>
                        <v-textarea class="custom-textarea" clearable auto-grow clear-icon="mdi-close-circle"
                            label="hex convert" v-model="convertHexColorText"></v-textarea>
                    </v-col>
                    <v-col>
                        <v-textarea class="custom-textarea" clearable auto-grow clear-icon="mdi-close-circle"
                            label="rgb convert" v-model="convertRgbColorText"></v-textarea>
                    </v-col>
                </v-row>
            </v-container>
            ...
            <v-container class="container-row" style="width: 600px;">
                <v-row style="display: flex; text-align: center; justify-content: center; margin: 40px;">
                    ...
                    <v-textarea clearable auto-grow clear-icon="mdi-close-circle" v-model="colorText"
                        disabled></v-textarea>
                </v-row>
            </v-container>
            <div>
